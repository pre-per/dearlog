/// Supertonic 2 ONNX TTS helper — ported from
/// https://github.com/supertone-inc/supertonic/blob/main/flutter/lib/helper.dart
///
/// Differences from upstream:
/// - logger → debugPrint
/// - WAV bytes builder (`encodeWavBytes`) added so the audio can flow through
///   the existing `Uint8List`-based player pipeline without a file round-trip.
/// - `copyModelToFile` writes to `getApplicationCacheDirectory()` once;
///   ONNX runtime keeps the session loaded after the first call.
library;
...
Future<TextToSpeech> loadTextToSpeech(String onnxDir) async {
  debugPrint('[Supertonic] Loading TTS models from $onnxDir');
  ...
  debugPrint('[Supertonic] TTS models loaded');
  return TextToSpeech(
    cfgs,
    textProcessor,
    sessions['dpOrt']!,
...
Future<Style> loadVoiceStyle(List<String> paths) async {
  ...
  final firstJson = jsonDecode(
    paths[0].startsWith('asset/') || paths[0].startsWith('assets/')
  ...
  for (var i = 0; i < bsz; i++) {
    final json = jsonDecode(
      paths[i].startsWith('asset/') || paths[i].startsWith('assets/')
...
Future<String> _copyModelToFile(String assetPath) async {
  ...
  final modelPath = '${tempDir.path}/${assetPath.split("/").last}';
  final file = File(modelPath);
  // Skip rewrite if cached file is already the right size — saves ~100MB of
  // disk churn on every cold start.
...
Future<Map<String, OrtSession>> _loadOnnxAll(String dir) async {
  ...
  final sessions = await Future.wait(models.map((name) async {
    ...
    debugPrint('[Supertonic] loading $name.onnx');
...
/// Encode a normalized [-1.0, 1.0] PCM stream into 16-bit little-endian
/// mono WAV bytes (RIFF/WAVE header included). Returned bytes are directly
/// playable by `just_audio` via `setFilePath` after writing or via in-memory
/// data sources.
Uint8List encodeWavBytes(List<double> audioData, int sampleRate) {
  const numChannels = 1;
  ...
  for (final byte in [0x52, 0x49, 0x46, 0x46]) {
    buffer.setUint8(offset++, byte);
  }
  buffer.setUint32(offset, 36 + dataSize, Endian.little);
  offset += 4;
  for (final byte in [0x57, 0x41, 0x56, 0x45]) {
    buffer.setUint8(offset++, byte);
  }
  for (final byte in [0x66, 0x6D, 0x74, 0x20]) {
    buffer.setUint8(offset++, byte);
  }
  ...
  offset += 2;
  for (final byte in [0x64, 0x61, 0x74, 0x61]) {
    buffer.setUint8(offset++, byte);
  }
